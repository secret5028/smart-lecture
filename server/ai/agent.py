...
class LectureAgent:
    ...
    async def get_recommendations(self) -> list[dict]:
        ...
        # compose_sequence에 넘길 통합 context 구성
        sequence_context = {
            "subject": subject,
            "target_audience": target_audience,
            "current_section": current_section,
            "transcript_recent": raw_query,   # 키워드 아닌 원문 발화 사용
            "progress_pct": progress_pct,
            "goal": goal,
            "toc_remaining": section_titles[current_idx + 1:current_idx + 5],
        }

        if chunks and gemini_api_key:
            dynamic_slides = await asyncio.to_thread(
                self.composer.compose_sequence,
                chunks,
                sequence_context,
                gemini_api_key,
            )
        else:
            dynamic_slides = self.composer._fallback_sequence(chunks, sequence_context)

        # 각 슬라이드에 공통 메타 정보 주입
        for slide in dynamic_slides:
            slide.setdefault("image_id", None)
            slide.setdefault("source_chunk_ids", [c.get("id") for c in chunks[:3] if c.get("id")])
            slide["subject"] = subject
            slide["current_section"] = current_section
            slide["progress_pct"] = progress_pct

        return (base_slides + dynamic_slides)[:RECOMMEND_COUNT]
